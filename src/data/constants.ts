
export const Bio = {
  avatar: 'https://media.licdn.com/dms/image/v2/D5603AQHGRX6J_XK6Ig/profile-displayphoto-shrink_800_800/profile-displayphoto-shrink_800_800/0/1708592954972?e=1740614400&v=beta&t=FF-iX1KZ7gmUPKvr7Yr8fCguI_ComIQCypVZ07nJQqE',
  name: "Vinh Nguyen",
  roles: [
    "Frontend Developer",
    "Nodejs Developer",
    "Programmer",
  ],
  description:
    "A dedicated developer skilled in modern frontend and backend technologies, focused on creating impactful, user-friendly solutions while continuously expanding expertise through learning and collaboration.",
  resume: "https://drive.google.com/file/d/1ffZrcMcn8UatXGIaautbbqpV7ADNaETA/view?usp=sharing",
  github: "https://github.com/khanhvinhnguyen",
  linkedin: "https://www.linkedin.com/in/khanhvinhnguyen/",
  email: "vinhnk.work@gmail.com",
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "React Js",
        image: "https://www.vectorlogo.zone/logos/reactjs/reactjs-icon.svg",
      },
      {
        name: "Next Js",
        image: "https://www.vectorlogo.zone/logos/nextjs/nextjs-icon.svg",
      },
      {
        name: "Redux",
        image: "https://www.vectorlogo.zone/logos/js_redux/js_redux-icon.svg",
      },
      {
        name: "HTML",
        image: "https://www.vectorlogo.zone/logos/w3_html5/w3_html5-icon.svg",
      },
      {
        name: "CSS",
        image:
          "https://www.vectorlogo.zone/logos/w3_css/w3_css-icon~old.svg",
      },
      {
        name: "JavaScript",
        image:
          "https://www.vectorlogo.zone/logos/javascript/javascript-icon.svg",
      },
      {
        name: "TypeScript",
        image:
          "https://www.vectorlogo.zone/logos/typescriptlang/typescriptlang-icon.svg",
      },
      {
        name: "Tailwind",
        image: "https://www.vectorlogo.zone/logos/tailwindcss/tailwindcss-icon.svg",
      },
      {
        name: "Material UI",
        image:
          "data:image/png;base64,",
      },
    ],
  },
  // Back-end
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://www.vectorlogo.zone/logos/nodejs/nodejs-icon.svg",
      },
      {
        name: "Express Js",
        image: "https://www.vectorlogo.zone/logos/expressjs/expressjs-icon.svg",
      },
      {
        name: "Nest Js",
        image: "https://www.vectorlogo.zone/logos/nestjs/nestjs-icon.svg",
      },
      {
        name: "GraphQl",
        image: "https://www.vectorlogo.zone/logos/graphql/graphql-icon.svg",
      },
      {
        name: "Python",
        image:
          "https://www.vectorlogo.zone/logos/python/python-icon.svg",
      },
      {
        name: "MySQL",
        image: "https://www.vectorlogo.zone/logos/mysql/mysql-icon.svg",
      },
      {
        name: "Postgresql",
        image: "https://www.vectorlogo.zone/logos/postgresql/postgresql-icon.svg",
      },
      {
        name: "MongoDB",
        image: "https://www.vectorlogo.zone/logos/mongodb/mongodb-icon.svg",
      },
      {
        name: "Firebase",
        image: "https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg",
      },
    ],
  },
  // Orthers
  {
    title: "Others",
    skills: [
      {
        name: "Git",
        image:
          "https://www.vectorlogo.zone/logos/git-scm/git-scm-icon.svg",
      },
      {
        name: "GitHub",
        image: "https://www.vectorlogo.zone/logos/github/github-tile.svg",
      },
      {
        name: "VS Code",
        image: "https://www.vectorlogo.zone/logos/visualstudio_code/visualstudio_code-icon.svg",
      },
      {
        name: "Postman",
        image: "https://www.vectorlogo.zone/logos/getpostman/getpostman-icon.svg",
      },
      {
        name: "Vercel",
        image: "https://www.vectorlogo.zone/logos/vercel/vercel-icon.svg",
      }
    ],
  },
];

export const experiences = [
  {
    id: 0,
    img: "https://avatars.githubusercontent.com/u/102339598?s=200&v=4",
    role: "Fullstack Developer",
    company: "bcPro Foundation",
    date: "Aug 2020 - Sep 2022",
    desc: "Contributed to two key projects under BCPRO Foundation: AbcPay, a multi-currency crypto wallet featuring advanced security and multi-signature capabilities, and Lixi.social, a decentralized social network designed to empower users with complete control over their experience, free from platform influence or ads.",
    skills: [
      "Next Js",
      "Node Js",
      "Redux Saga",
      "Antd",
      "Styled-components",
      "NestJS",
      "PostgreSQL",
      "GraphQL",
      "Redis",
      "TypeScript",
    ],
  },
  {
    id: 1,
    img: "https://cdn-new.topcv.vn/unsafe/140x/https://static.topcv.vn/company_logos/digiloinc-ab8198c970b4dcb51f4a6c1e9085c166-5e6a2c6cd862e.jpg",
    role: "Frontend Developer",
    company: "Digilo",
    date: "Aug 2024 - Dec 2024",
    desc: "Developed and enhanced web applications for project management platforms like ShareGantt and Infolio. Focused on creating intuitive interfaces using modern frontend technologies, improving user experience, and ensuring seamless functionality for task management, collaboration, and file integration.",
    skills: [
      "HTML",
      "CSS",
      "JavaScript",
      "ReactJS",
      "Vuejs",
      "Redux",
      "Material UI",
    ],
  },
  {
    id: 2,
    img: "https://firebasestorage.googleapis.com/v0/b/flexi-coding.appspot.com/o/girlScript.jpeg?alt=media&token=e656a621-cf3c-4230-bf0f-e74b4cec6035",
    role: "Open Source Contributor",
    company: "Freelance",
    date: "Jun 2023 - Present",
    desc: "Contributing to different open-source projects and learn from industry experts",
  },
];

export const educations = [
  {
    id: 0,
    img: "https://upload.wikimedia.org/wikipedia/commons/thumb/0/00/Logo_UIT_updated.svg/742px-Logo_UIT_updated.svg.png",
    school: "University of Information Technology, Ho Chi Minh City",
    date: "Sep 2017 - Jul 2022",
    desc: "I am currently pursuing a Bachelor's degree in Computer Science and Engineering at Kalinga Institute of Industrial Technology, Bhubaneswar. I have completed 4 semesters and have a CGPA of 8.71. I have taken courses in Data Structures, Algorithms, Object-Oriented Programming, Database Management Systems, Operating Systems, and Computer Networks, among others. I am also a member of the Google Developers Student Club (GDSC) at KIIT, where I am learning and working on exciting projects with a team of talented developers.",
    degree: "Bachelor of Technology - BTech, Computer Science and Engineering",
  },
  {
    id: 1,
    img: "https://scontent.fhan3-5.fna.fbcdn.net/v/t39.30808-1/285790288_412721004068006_6441068186446103437_n.jpg?stp=dst-jpg_s480x480_tt6&_nc_cat=109&ccb=1-7&_nc_sid=2d3e12&_nc_ohc=0yYWjd_UO9IQ7kNvgFosPVy&_nc_oc=AdiaiU4ue8oNqeeTs4G_Uyzf0sw0fgGyU7RVWeUkaWqxi6M9gSwlVKWqeT1_XVxCn1N-CSUxwHDoRINcFEIm2em1&_nc_zt=24&_nc_ht=scontent.fhan3-5.fna&_nc_gid=ADEPwHhD3Dgydtmvw-j-Vn8&oh=00_AYAwdZwvSNlxjCNL_h_jnlbbj-G-4YH2JcDkhxf5ujtkVQ&oe=676DE344",
    school: "TOEIC LR",
    degree: "English Certificate",
    date: "Jan 2024",
    grade: "590/990"
  },
  {
    id: 2,
    img: "https://scontent.fhan3-5.fna.fbcdn.net/v/t39.30808-1/285790288_412721004068006_6441068186446103437_n.jpg?stp=dst-jpg_s480x480_tt6&_nc_cat=109&ccb=1-7&_nc_sid=2d3e12&_nc_ohc=0yYWjd_UO9IQ7kNvgFosPVy&_nc_oc=AdiaiU4ue8oNqeeTs4G_Uyzf0sw0fgGyU7RVWeUkaWqxi6M9gSwlVKWqeT1_XVxCn1N-CSUxwHDoRINcFEIm2em1&_nc_zt=24&_nc_ht=scontent.fhan3-5.fna&_nc_gid=ADEPwHhD3Dgydtmvw-j-Vn8&oh=00_AYAwdZwvSNlxjCNL_h_jnlbbj-G-4YH2JcDkhxf5ujtkVQ&oe=676DE344",
    school: "TOEIC SW",
    degree: "English Certificate",
    date: "Mar 2024",
    grade: "230/400"
  },
];

export const projects = [
  {
    id: 0,
    title: "Dieu Thien Resrtaurant",
    date: "Jun 2024",
    description:
      "A sleek and responsive vegetarian landing page built with Next.js and Ant Design, featuring dynamic navigation, elegant UI components, and fast-loading performance to enhance user engagement.",
    image:
      "https://firebasestorage.googleapis.com/v0/b/portfolio-35084.appspot.com/o/DieuThienBanner.jpg?alt=media&token=94a79ff9-a581-4990-8d6e-eeccf491c29c",
    tags: [
      "Nextjs",
      "Next-int",
      "React-Hook-Form",
      "Ant Design",
      "Framer Motion",
      "Firebase",
      "Typescript",
    ],
    category: "web app",
    github: "",
    webapp: "https://amthucchaydieuthien.com/",
    member: [{
      img: 'https://media.licdn.com/dms/image/v2/D5635AQH-T4uKaDSHIQ/profile-framedphoto-shrink_800_800/profile-framedphoto-shrink_800_800/0/1733931495533?e=1735549200&v=beta&t=xWMM25xqHJU6dkzPC8HgPT-tjGh9LUl-BlVFpP0qxGk',
      name: 'My Lưu',
      linkedin: 'https://www.linkedin.com/in/my-luu/',
      github: 'https://github.com/mayluu',
    }]
  },
  {
    id: 1,
    title: "Solar System",
    date: "May 2024",
    description:
      "A dynamic and interactive web application that visualizes the movements and details of celestial bodies in our solar system, including planets, moons, and the International Space Station (ISS). The platform provides basic astronomical information and real-time planetary motion using advanced 3D rendering.",
    image:
      "https://firebasestorage.googleapis.com/v0/b/portfolio-35084.appspot.com/o/solar-system.png?alt=media&token=4a99976e-0f5a-41b1-b3e4-30a3d1829d53",
    tags: [
      "ReactJS",
      "Vite",
      "ThreeJS",
      "Ant Design",
      "React-intl",
    ],
    category: "web app",
    github: "https://github.com/khanhvinhnguyen/Solar-system",
    webapp: "https://solar-system-simple.vercel.app/",
  },
  
];

